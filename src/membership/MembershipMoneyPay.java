...
public class MembershipMoneyPay extends JFrame{
	
	private static final long serialVersionUID = 1L;
	boolean check=false;

	public MembershipMoneyPay(){
		setTitle("SONA 현금 결제");
		...
		String str=Integer.toString(BuyMembership.price)+"\\";
		howmuch.setText(str);
		...
		TimerTask task3=new TimerTask() {
			...
			public void run() {
				if(check==false) {
					...
					BuyMembership bm=new BuyMembership();
					bm.dispose();
					dispose();
					MembershipReceipt rc = null;
					try {
						rc = new MembershipReceipt();
						rc.setVisible(true);
