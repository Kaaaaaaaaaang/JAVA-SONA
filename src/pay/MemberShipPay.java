...
public class MemberShipPay extends JFrame{
	...
	public MemberShipPay() {
		...
		String str=MemberPay.time+"시간   "+Integer.toString(MemberPay.price)+"\\";
		howmuch.setText(str);
		...
		if(user.getMem_money()!=0) {
			...
			money.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					try {
						if(main.MainScreen.db.minusmoney(MemberPay.price)==true) {
		...
		if(user.getMem_time()!=null) {
			...
			time.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					try {
						if(main.MainScreen.db.minustime(MemberPay.time)==true) {
							dispose(); pay.Receipt r=new pay.Receipt(); r.setVisible(true);
							JOptionPane.showMessageDialog(null, "결제가 완료되었습니다.");
						}else {
							JOptionPane.showMessageDialog(null, "잔액이 부족합니다.");
